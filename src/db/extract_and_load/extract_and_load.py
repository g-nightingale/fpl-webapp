...
from db_helpers import create_table_if_not_exists, truncate_table, drop_table, store_data_in_rds, run_query, get_current_datetime
...
def get_fixtures_data():
    ...
    if response.status_code == 200:
        ...
        body_str = f"Failed to fetch content from {URL}. Status code: {response.status_code}"
        print(body_str)
        print(response)
        return []
        
    current_datetime = get_current_datetime()
    print(f'Fixtures data length {len(content)}')
    fixtures_data = []
...
def get_teams_data():
    ...
    print(f'Teams array length {len(teams)}')
    teams_data = []
...
def get_players_data():
    ...
    print(f'Players data length: {len(players)}')
...
def get_players_historical_data():
    ...
    print(f'Number of players to collect data for: {number_of_players}')
    ...
    for i in range(number_of_players):
        print(f'Collecting historical data for player {i}')
...
def get_element_type_data():
    ...
    print(f'Elements data length: {len(elements)}')
...
def update_player_historical_data():
    ...
    raw_player_history_data = get_players_historical_data()
    if len(raw_player_history_data) > 0:
        create_table_if_not_exists(CREATE_TABLE_QUERY)
        truncate_table(TABLE_NAME)
        store_data_in_rds(INSERT_QUERY, raw_player_history_data)
    else:
